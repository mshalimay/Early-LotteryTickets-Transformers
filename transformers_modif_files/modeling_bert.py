...
class BertSelfAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        ...
        self.slimming = getattr(config, 'inter_slimming', False)
        self.pruning_ratio = getattr(config, 'iner_pruning_ratio', 0.0)
        self.slim_before = getattr(config, 'slim_before', False)
        self.soft_by_one = getattr(config, 'soft_by_one', False)
        ...
        if self.slimming:
            self.slimming_coef = nn.Parameter(torch.ones(self.num_attention_heads).reshape(1,-1,1,1) * 1.0)
    ...
    def forward(
        ...
        if self.slimming and self.slim_before:
            attention_scores = attention_scores * self.slimming_coef

        # Normalize the attention scores to probabilities.
        if self.soft_by_one:
            attention_probs = softmax_off_by_one(attention_scores, dim=-1)
        else:
        ...
        if self.slimming and not self.slim_before:
            attention_probs = attention_probs * self.slimming_coef
...
class BertSelfOutput(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.pruning_ratio = getattr(config, 'self_pruning_ratio', 0.0)
        self.all_head_size = round(config.hidden_size * (1 - self.pruning_ratio))
...
class BertAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        super().__init__()
        self.slimming = getattr(config, 'slimming', False)
    ...
    def prune_heads(self, heads):
        if len(heads) == 0:
            return
        #NOTE @modif
        heads, index, slimming_index = find_pruneable_heads_and_indices(
            heads, self.self.num_attention_heads, self.self.attention_head_size, self.pruned_heads, nn_mode=self.self
        )        
...
class BertLayer(nn.Module):
    def __init__(self, config):
        ...
        #NOTE @modif
        self.slimming = getattr(config, 'self_slimming', False)
        if self.slimming:
